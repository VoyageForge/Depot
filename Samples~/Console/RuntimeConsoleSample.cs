...
namespace VoyageForge.Depot.Samples.Console
{
    ...
    ///
    /// 中文字体说明：
    /// 运行时默认字体不含中文字形，会导致控制台中文不显示。本 Sample 的
    /// Resources/Depot/Console/Fonts 目录提供了 Noto Sans SC 字体资源，
    /// 导入 Sample 后即可通过 Resources.Load 直接加载，无需手动移动。
    /// </summary>
    public static class RuntimeConsoleBootstrap
    {
        /// <summary>
        /// 中文字体文本设置的 Resources 路径（可选）。
        /// 字体文件随 Sample 放在 Resources/Depot/Console/Fonts 下，默认路径即可加载；
        /// 换成你自己的字体资源时填写对应路径。留空则不加载自定义字体。
        /// </summary>
        private const string TextSettingsResourcePath = "Depot/Console/Fonts/UITK Text Settings";
